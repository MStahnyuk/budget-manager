        ...
        if(item) {
            return (
                <form className={"form " + active}>
                    ...
                    <button className="close"  onClick={(event)=> this.props.onCloseForm(event, 'categories', this.props.item.id, form.id)}><i className="fa fa-times"></i></button> 
                </form>
            )
        }
        return null
...
function mapDispatchToProps(dispatch) {
    return {
        // onRadioChange: (event) => dispatch(changeCategoryType(event.target.value)),
