...
class FormEditNote extends React.Component {
    render() {
        const {active, form,  onCreateNote, item, onCloseForm } = this.props;
        if(item) {
            return (
                <form className={"form " + active}>
                        ...
                        <label>
                            Note Title:
                            <Input placeholder='Note name' fieldChange={'title'} value={item.title} itemId={item.id} arrayChange='notes' />
                        </label>

                        <label>
                            Note Value:
                            <Input placeholder='Note name' inputType='number' fieldChange={'value'} value={item.value} itemId={item.id} arrayChange='notes'/>
                        ...
                        <button className="close"  onClick={(event)=> onCloseForm(event, 'notes', this.props.item.id, form.id)}><i className="fa fa-times"></i></button> 
                    </form>
            )
        }
        return null
